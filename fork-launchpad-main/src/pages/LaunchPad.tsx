...
import { Search, Filter, Rocket, Trophy, X, ChevronDown } from 'lucide-react';
...
export default function LaunchPad() {
  ...
  const { projectFilters, setProjectFilters, getFilteredProjects } = useAppStore();
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">
                        {Math.round((project.fundingRaised / project.fundingAsk) * 100)}%
                      </span>
                    </div>
                    <div className="h-2 bg-muted rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-primary rounded-full transition-all duration-500"
                        style={{ width: `${Math.min(100, (project.fundingRaised / project.fundingAsk) * 100)}%` }}
                      />
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="currency-ada">₳{project.fundingRaised.toLocaleString()}</span>
                      <span className="text-muted-foreground">of ₳{project.fundingAsk.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </Link>
