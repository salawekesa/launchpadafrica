...
import {
  ...
  Clock, CheckCircle2, AlertCircle, Wallet, Upload, User, Building
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useAuthStore } from '@/store/auth-store';
import { useAppStore } from '@/store/app-store';
import { useMyStartups } from '@/hooks/useStartupsFromApi';
...
export default function Dashboard() {
  const { user, isAuthenticated, activeProfile, setActiveProfile } = useAuthStore();
  const { projects, activities, getPledgesByInvestor } = useAppStore();
  const { projects: myStartupsFromApi, isLoading: loadingMyStartups } = useMyStartups(user?.id ?? null);

  if (!isAuthenticated || !user) {
    return <Navigate to="/login" replace />;
  }

  const isBuilder = activeProfile === 'builder';
  const isInvestor = activeProfile === 'investor';
  const userProjects = isBuilder ? myStartupsFromApi : [];
    ...
    <div className="container py-8">
      ...
      >
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground">
                {isBuilder
                  ? "Here's your startups and activities"
            ...
            <div className="flex items-center gap-2 p-1 rounded-lg bg-muted/50 border border-border">
              <button
                ...
                className={cn(
                  ...
                  isBuilder ? 'bg-background shadow text-primary' : 'text-muted-foreground hover:text-foreground'
                )}
              >
              ...
              <button
                ...
                className={cn(
                  'flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-colors',
                  isInvestor ? 'bg-background shadow text-primary' : 'text-muted-foreground hover:text-foreground'
                )}
              >
        ...
        <motion.div variants={fadeUp} className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {isBuilder ? (
            <>
              <StatCard icon={Rocket} label="Your Projects" value={userProjects.length.toString()} />
              <StatCard icon={Trophy} label="Hackathons Won" value={user.hackathonsWon.length.toString()} />
              <StatCard icon={Users} label="Collaborations" value={user.collaborations.length.toString()} />
              <StatCard
                icon={CheckCircle2}
                label="Verification"
            ...
            <>
              <StatCard icon={TrendingUp} label="Total Pledged" value={`₳${totalPledged.toLocaleString()}`} highlight />
              <StatCard icon={Rocket} label="Projects Backed" value={pledgedProjects.length.toString()} />
              <StatCard icon={Wallet} label="Wallet" value={user.walletAddress ? 'Connected' : 'Not Connected'} highlight={!!user.walletAddress} />
              <StatCard
                icon={CheckCircle2}
                label="Verification"
                value={user.verificationLevel === 'level2' ? 'Level 2' : user.verificationLevel === 'level1' ? 'Level 1' : 'Unverified'}
            ...
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold">
                  {isBuilder ? 'Your Startups' : 'Pledged Projects'}
                </h2>
              ...
              {isBuilder && loadingMyStartups ? (
                <div className="py-12 text-center text-muted-foreground">Loading your startups...</div>
              ) : (isBuilder ? userProjects : pledgedProjects).length > 0 ? (
                <div className="space-y-4">
                  {(isBuilder ? userProjects : pledgedProjects).slice(0, 5).map((project) => project && (
                    <Link key={project.id} to={isBuilder ? `/startup/${project.id}` : `/project/${project.id}`}>
                      <div className="p-4 rounded-xl border border-border hover:border-primary/50 transition-colors">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <span className={`stage-badge ${STAGE_CONFIG[project.stage]?.className ?? ''}`}>
                            {STAGE_CONFIG[project.stage]?.label ?? project.stage}
                          </span>
                        </div>
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="font-medium mb-2">
                    {isBuilder ? 'No startups yet' : 'No pledges yet'}
                  </h3>
                  <p className="text-sm text-muted-foreground mb-4">
                    {isBuilder
                      ? 'Upload your startup to the Launch Pad and get it listed.'
                      : 'Browse projects and make your first pledge'
                    }
                  </p>
                  <Link to={isBuilder ? '/submit' : '/launchpad'}>
                    <Button size="sm" className={cn(isBuilder && 'gap-2')}>
                      {isBuilder ? (
                        <>
                          <Upload className="w-4 h-4" />
                          Submit your startup
