...
export default function Landing() {
  const { projects: apiProjects } = useStartupsFromApi();
  const { hackathons } = useAppStore();

  const activeProjects = apiProjects.filter(p => p.stage !== 'archived');
  const fundedProjects = apiProjects.filter(p => p.stage === 'funded' || p.stage === 'scaling');
  const totalFundingRaised = apiProjects.reduce((sum, p) => sum + p.fundingRaised, 0);
